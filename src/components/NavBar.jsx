      ...
      <Nav className='d-flex gap-2 w-100'>
        ...
        <Nav.Item className='endPart'>
         ...
         {currentUser&&currentUser?.role!=="admin" && (<> <div  onClick={()=>openRequest.mutate()} className="notification position-relative">
            <BsPeopleFill id="request" onClick={()=>openRequest.mutate()}   className='icon' role="button" size={25}/>
           ...
           <div    className="notification position-relative">
            <BsBellFill id="notification" onClick={()=>openNotification.mutate()}   className='icon' role="button" size={25}/>
        ...
        <div   className="hamburger jsutify-self-end">
          <div onClick={()=>setShowDropDown(!showDropDown)}  id="ham" className='hamInner' >
          ...
          <div className="line"></div>
            ...
            <div ref={themeRef} className={`d-flex flex-column position-absolute drop ${showDropDown?"show":""}`}>
             ...
             {currentUser && (<><Link className='link' to="/write"><div className='d-flex gap-1 mt-1'><strong>Write</strong><TbEditCircle/> </div></Link>
              <Link className='link' to="/settings"><div className='d-flex gap-1 mt-1'><strong>Settings</strong><TbEditCircle/> </div></Link><Button onClick={logoutHandler} className='align-self-center'>Log Out</Button></>)} 
<div className="mobileInp d-flex "><Form.Control placeholder={t("search")} type="text" value={search} onChange={e=>setSearch(e.target.value)}/><Button onClick={searchHandler}><BiSearch/></Button></div>
