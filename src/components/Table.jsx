...
import ReactIndiana from "react-indiana-drag-scroll"

import {SelectCheckbox} from "../components"
import { axiosFetch } from '../axios'
import { useTranslation } from 'react-i18next'

const TableComponent = ({subject,columns,data,users,deleted,admins,refetch}) => {
  ...
  return (
    <div className='d-flex flex-column'>
      {users && (
        ...
        <Button onClick={()=>makeAdminOrNonAdminMutation.mutate({userIds,role:"admin"})} variant="success">{t("make_admin")}</Button>
      ...
      {deleted && (
        <div className='align-self-end mb-2'>
          <Button onClick={()=>blockOrUnblockMutation.mutate({userIds,status:"active"})}>{t("reactivate")}</Button>
        </div>
      )}
      {admins && (
        <div className="align-self-end mb-2">
           <Button onClick={()=>makeAdminOrNonAdminMutation.mutate({userIds,role:"user"})} variant="success">{t("make_nonadmin")}</Button>
        </div>
      )}
      
     <Table id="table" responsive bordered hovered="true" striped {...getTableProps()}>
      <thead>
    ...
    <div className="bottom">
            <div className="d-flex">
              <Button disabled={!canPreviousPage} onClick={()=>previousPage()}>{t("previous")}</Button>
              <Form.Control type="number" defaultValue={1} onChange={e=>{
                const pageIndex=+e.target.value-1;
                gotoPage(pageIndex);

              }}/>
              <Button disabled={!canNextPage} onClick={()=>nextPage()}>{t("next")}</Button>
            </div>
            <p className='text-center mt-3'><strong>{pageIndex+1}</strong> {t("of")} {pageOptions.length}</p>
            <Button disabled={!data?.length} onClick={download} className='d-flex gap-1' variant="link"><ImFilePdf/>{t("download")}</Button>
    </div>
