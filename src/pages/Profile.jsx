...
const Profile = () => {
  ...
  return (
    <div className="profile">
      <div className="left">
        <div className="leftTop d-flex flex-column gap-3">
         ...
         {isLoading ? <ClipLoader size={80}/>: <Image thumbnail="true" src={file?URL.createObjectURL(file):profUpdated? profileImg:keys.PF+profileImg}/>}
          ...
          <p><strong>{t("number_of_reviews")} </strong>{numberOfReviews}</p>
        
          <p><strong>{t("rating_number")}: </strong>{ratingNumber}</p>
        
          <p><strong>{t("email")}: </strong>{email}</p>
          <div className='d-flex gap-2'><strong>{t("bio")}: </strong>
          {isLoading? <ClipLoader size={40}/>:inEditMode? <Form.Control onChange={e=>setText(e.target.value)} value={text} as="textarea"/>: <div className="bio">{bio?bio:t("no_bio")}</div>}  
          </div>
          <p><strong>{t("member_since")}: </strong>{moment(createdAt).format("L")}</p>
          <Button onClick={inEditMode?()=>mutate({bio:text}):()=>setInEditMode(true)} className='d-flex align-items-center gap-1 justify-content-center'>{inEditMode?"Save":<><TiEdit size={20}/>{t("update")}</>}</Button>
         
        </div>

        <div className="leftBottom">
        <h1>{firstName} {t("reviews_own")}</h1>
          ...
          {reviews.slice(0,2).map(review=>(
              ...
              <div className="mini-review">
                  ...
                  <p className='mt-2'><strong>{t("review_name")}:</strong> {review.reviewName}</p>
                  <p className='mt-2'><strong>{t("average_rating")}:</strong> {review.averageRating}</p>
              </div>
              </Link>
            ))}
          </div>
      ...
      <div className="right">
        <h1>{userProfile?.firstName}  {t("reviews_own")} </h1>
        <hr/>
        <TableComponent subject={currentUser.uuid} data={userProfile?.reviews} columns={COLUMNS}/>
        </div>
